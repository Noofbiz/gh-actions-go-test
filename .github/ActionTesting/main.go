...
func main() {
	...
	if err = checkForCoverageChanged.Start(); err != nil {
		log.Fatalf("Unable to start git status Error was: %v", err.Error())
	}
	gitBytes, _ := io.ReadAll(gitOutput)
	coverMatch, err := regexp.Match(`\.github\/coverage.out`, gitBytes)
	if err != nil {
		log.Fatalf("Unable to match git status output with [[ coverage.out ]]. Error was: %v", err.Error())
	}
	svgMatch, err := regexp.Match(`\.github\/coverage\.svg`, gitBytes)
	if err != nil {
		log.Fatalf("Unable to match git status output with [[ coverage.svg ]]. Error was: %v", err.Error())
	}
...
func commitToPR() error {
	...
	pushOutput, _ := gitPush.StderrPipe()
